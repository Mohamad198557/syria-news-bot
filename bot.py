...
# 3. المصادر
TELEGRAM_CHANNELS = ["AlekhbariahSY", "SyPresidency", "SyMOIGov", "syrianmo", "syrianmofaex1"]
...
def get_syria_weather():
    ...
    # تم إصلاح الخطأ البرمجي في القاموس هنا
    ...
    try:
        ...
        if lines: return report + "  ".join(lines[:3]) + "\n" + "  ".join(lines[3:]) + "\n\n"
    except: pass
    return ""
...
def fetch_telegram_channel(user, sent):
    ...
    try:
        ...
        for m in msgs[::-1][:5]:
            ...
            news.append({'title': content[:150], 'link': link, 'source': source_name, 'image': img, 'is_brk': is_brk})
    except: pass
    return news
...
def send_telegram_msg(chat_id, text, image=None, is_breaking=False):
    ...
    payload = {"chat_id": chat_id, "parse_mode": "HTML", "disable_notification": not is_breaking}
    if image:
        url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto"
        payload.update({"photo": image, "caption": text[:1024]})
    else:
...
def main():
    ...
    links_to_save = []
    tg_brk, tg_nrm, rss_brk, rss_nrm = [], [], [], []

    with ThreadPoolExecutor(max_workers=15) as ex:
        tg_f = [ex.submit(fetch_telegram_channel, ch, sent_list) for ch in TELEGRAM_CHANNELS]
        rss_f = [ex.submit(fetch_feed, u, sent_list) for u in RSS_FEEDS]
        for f in as_completed(tg_f):
            for i in f.result():
                if i.get('is_brk'): tg_brk.append(i)
                else: tg_nrm.append(i)
        for f in as_completed(rss_f):
            b, n = f.result(); rss_brk.extend(b); rss_nrm.extend(n)

    all_brk = tg_brk + rss_brk
    for b in all_brk:
        msg = f"🚨 <b>خبر عاجل</b>\n\n📰 <b>{b['title']}</b>\n\nالمصدر: {b['source']} | <a href='{b['link']}'>🔗 الرابط</a>"
        for cid in TARGET_CHATS:
            if send_telegram_msg(cid, msg, image=b.get('image'), is_breaking=True): links_to_save.append(b['link'])

    all_nrm = tg_nrm + rss_nrm
    if all_nrm:
        dam_time = (datetime.utcnow() + timedelta(hours=3))
        msg = f"<b>🇸🇾 موجز الشارع السوري</b>\n📅 {dam_time.strftime('%Y/%m/%d')} | {get_hijri_date()}\n✨ <i>\"{random.choice(DAILY_WISDOM)}\"</i>\n───━━━━─━━━━───\n\n"
